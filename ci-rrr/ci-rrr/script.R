...
# Hmm, it sees a smoothe number (Age) and tries to smoothe the color.  Let's turn age into labels so ggplot2 treats them like different categories:
df$age_string = as.character(df$Age)
ggplot(df, aes(Foot_length_cm, Reading_grade, color = age_string)) + geom_point()

# Chart's looking a bit muddled.  Let's look at separate charts
